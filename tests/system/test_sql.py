...
from examples import sql_testing
...
class TestPackage(unittest.TestCase):
    ...
    def test_testing(self):
        ...
        # Run function
        storages.sql.run(sql_testing.url, sql_testing.prefix, sql_testing.source, self.target, 'testing')

        # Assert schemas
        source = json.load(io.open(sql_testing.source, encoding='utf-8'))
        target = json.load(io.open(self.target, encoding='utf-8'))
        assert source == target

        # Assert data
        for source, target in zip(source['resources'], target['resources']):
            spath = os.path.join(os.path.dirname(sql_testing.source), source['path'])
